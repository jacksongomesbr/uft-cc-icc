...
## Por que a história entra em uma disciplina técnica

Este capítulo não existe para vocês decorarem nomes e datas. Ele existe porque
quase toda decisão técnica que vocês vão encontrar no curso tem uma explicação
histórica, e sem ela a decisão parece arbitrária.
...
A pergunta "qual foi o primeiro computador?" aparece em toda conversa sobre
este assunto e não tem resposta única. A pergunta não é difícil. Ela é mal
formulada.

@wazlawick2016 propõe substituí-la por uma matriz de critérios. Em vez de
disputar um pódio, declara-se sob que aspecto uma máquina vem antes das outras.
A @tbl-criterios-primeiro lista os cinco critérios que separam as candidatas do
período da Segunda Guerra, e a seção sobre os primeiros computadores os aplica.
...
### Cinco máquinas e nenhum campeão

A Segunda Guerra acelerou tudo. O período reúne máquinas com propósitos,
tecnologias e graus de programabilidade muito diferentes, e a lista abaixo
serve para ser cruzada com a @tbl-criterios-primeiro.

A série Z, desenvolvida por Konrad Zuse na Alemanha, chega com a Z3 a um marco
de automação eletromecânica programável, construída com relés.

Em Bletchley Park, no Reino Unido, o esforço de criptanálise produziu a Bombe e
depois o Colossus, máquinas especializadas em quebrar cifras. O Colossus é
eletrônico e não é de propósito geral: ele foi construído para uma tarefa.

O Atanasoff-Berry Computer, nos Estados Unidos, aplica eletrônica à solução de
sistemas de equações, também sem propósito geral.

O Harvard Mark I é eletromecânico, grande e operacional, usado para cálculos
durante anos.

![O Colossus, em Bletchley Park, operado por duas integrantes do Women's Royal Naval Service. Era eletrônico e construído para uma tarefa: quebrar cifras. Imagem em domínio público, obtida no Wikimedia Commons.](../diagrams/02/colossus.jpg){#fig-colossus width=82% fig-alt="Sala com um grande painel de equipamento eletrônico coberto de válvulas e cabos, ocupando toda a parede. Duas mulheres de uniforme trabalham diante do painel, uma delas manipulando um rolo de fita."}

O ENIAC é eletrônico, de propósito geral e operacional, e sua programação era
feita por cabos e chaves. Trocar o problema significava reconectar a máquina, o
que levava dias. A @fig-colossus e a @fig-eniac mostram duas dessas máquinas, e
as duas fotografias registram também quem as operava.

![O ENIAC, na Universidade da Pensilvânia. A programação era feita conectando cabos e ajustando chaves, e não escrevendo instruções na memória. Fotografia de TexasDex, licenciada em CC BY-SA 3.0, obtida no Wikimedia Commons.](../diagrams/02/eniac.jpg){#fig-eniac width=82% fig-alt="Painéis altos de equipamento eletrônico alinhados ao longo de uma parede, cobertos de mostradores circulares, cabos e conectores. Uma pessoa em pé opera um dos painéis."}

Nenhuma dessas máquinas vence em todos os cinco critérios, e é por isso que a
pergunta sobre a primeira não fecha. Cada uma vem antes sob um aspecto. Somem-se
a isso as disputas de autoria, comuns em projetos militares e de laboratório,
em que o trabalho é coletivo e o crédito é atribuído depois, e vocês têm o
retrato de um período que resiste a listas simples.

A @fig-matriz-criterios aplica os cinco critérios às cinco máquinas. Leiam a
figura por coluna, e não por linha: cada coluna tem pelo menos uma resposta
favorável, e nenhuma linha do período da guerra é favorável em tudo.
...
```{mermaid}
%%| label: fig-camadas-web
%%| fig-cap: "As camadas envolvidas em uma página. Cada uma se apoia na de baixo e não se confunde com ela."
%%| fig-width: 6
%%| echo: false
flowchart LR
  S["Servico<br/>busca, venda,<br/>publicacao"]
  N["Navegador<br/>solicita e<br/>apresenta recursos"]
  W["Web<br/>URL, HTTP<br/>e hipertexto"]
  I["Internet<br/>transporta dados<br/>entre redes"]
  S --> N --> W --> I
```
...
O piso desta semana tem duas partes.

A primeira é um exercício de critério. Escolham uma das máquinas citadas na
seção sobre os primeiros computadores e preencham, para ela, as cinco linhas da
@tbl-criterios-primeiro, respondendo sim ou não a cada pergunta. Depois,
escrevam uma frase no formato "a máquina X foi a primeira sob o critério Y",
com a justificativa em uma linha. Tragam a frase escrita.

A segunda é um exercício de camadas. Escolham uma tecnologia que vocês usam
todo dia e separem as três camadas da @fig-tres-camadas: qual é o problema
humano, qual é a representação escolhida e qual é o mecanismo que a executa.
Uma linha para cada camada basta.

A extensão é para quem terminou o piso sem esforço, e também tem duas partes.

Peguem a tecnologia da segunda tarefa e procurem a data da ideia, a data da
primeira implementação e a data da adoção em escala. Se as três coincidirem,
provavelmente vocês encontraram só uma delas e as outras duas estão em outro
lugar. Anotem quanto tempo separou a primeira da última.

Depois, escolham uma afirmação sobre tecnologia que vocês tenham lido ou ouvido
nos últimos dias, em qualquer lugar, e classifiquem-na segundo a
@tbl-etiqueta-temporal. Digam se ela é marco documentado, tendência, previsão ou
afirmação sobre o estado atual, e o que seria necessário para verificá-la. Essa
é a mesma operação que o capítulo 15 vai pedir, e treiná-la agora, em um caso
pequeno, é mais barato que descobri-la na semana do seminário.
